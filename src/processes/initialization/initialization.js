...
module.exports = {
    ...
    startLogger: (client) => {
        ...
        // Logger.startLogCombinerTimer(); //TODO Get this working
    ...
    reactionInitialization: async (channel) => {
        ...
        for (const emoji of Object.values(reactionEmojis)) {
            Logger.log(
                `Checking for ${emoji} emoji on message: ${message.id}.`
            );
            if (!emojiExistOnMessage(message, emoji, (selfCheck = true))) {
                ...
                Logger.log(
                    `Successfully put ${emoji} on message: ${message.id}.`
                );
            } else {
                Logger.log(
                    `${emoji} already exists on message: ${message.id}.`
                );
            }
        }
    },
